...
?>
  ...
  <?php
include "vista/nav_cadete.php";
?>
<div class="pedidos">	
<div>
<div class="container">
  <h2>Usuario Cadete</h2>
		...
		<div class="modal-dialog modal-sm">
		  <div class="modal-content">
            ...
            <div class="row">
					...
					<div class="col-md-9">
						<input type="text" name="usuario_modal" id="usuario_modal" class="form-control-sm" required>
					</div>	
				</div>
				...
				<div class="row">
					...
					<div class="col-md-9">
						<input type="text" name="contra_modal" id="contra_modal" class="form-control-sm" required>
					</div>	
				</div>
...
$(document).ready(function() {
	...
	$(function () {
		$('#update_country').on('show.bs.modal', function (event) {
			...
			var modal = $(this);
            modal.find('#contra_modal').val(contraseña);
			modal.find('#usuario_modal').val(usuario);
			modal.find('#id_modal').val(id_user);
		});
    });
	$(document).on("click", "#update_data", function() { 
		$.ajax({
			url: "update_user_cadete.php",
			type: "POST",
			cache: false,
			data:{
				id_user: $('#id_modal').val(),
				usuario: $('#usuario_modal').val(),
                contraseña: $('#contra_modal').val(),
