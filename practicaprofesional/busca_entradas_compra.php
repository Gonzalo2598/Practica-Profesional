...
if($db->numero_de_registros($exe)>0){
 ...
 while($ww=$db->buscar_array($exe)){
     $numero=$ww['numero'];
     $monto=0.00;
     $consulta_resumen=$db->consulta("Select k.fecha,k.tipo,k.numero,k.user,k.proveedor,k.costou,k.cantidad,k.descuento_porcentaje,k.impuesto_porcentaje,p.nombre_empresa from kardex k, proveedores p
     where k.numero=$numero and k.proveedor=p.id_prov");
      while($datas=$db->buscar_array($consulta_resumen)){
          $entrada=$datas['numero'];
          $prov=$datas['nombre_empresa'];
